...
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/docs/how-to-write-a-module.md"
...
- [Clone](https://docs.github.com/en/github/creating-cloning-and-archiving-repositories/cloning-a-repository#:~:text=to%20GitHub%20Desktop-,On%20GitHub%2C%20navigate%20to%20the%20main%20page%20of%20the%20repository,Desktop%20to%20complete%20the%20clone.)
  locally the **forked** repository (e.g `git clone https://github.com/odyslam/go.plugin`).
- Using a terminal, `cd` into the directory (e.g `cd go.plugin`)
...
> [example module](https://github.com/khulnasoft/go.plugin/tree/master/modules/example), which will 
...
- Add the source code to [`modules/example2/`](https://github.com/khulnasoft/go.plugin/tree/master/modules).
    - [module interface](#module-interface).
    - [suggested module layout](#module-layout).
    - [helper packages](#helper-packages).
- Add the configuration to [`config/go/example2.conf`](https://github.com/khulnasoft/go.plugin/tree/master/config/go).
- Add the module to [`config/go.conf`](https://github.com/khulnasoft/go.plugin/blob/master/config/go.conf).
- Import the module in [`modules/init.go`](https://github.com/khulnasoft/go.plugin/blob/master/modules/init.go).
- Update the [`available modules list`](https://github.com/khulnasoft/go.plugin#available-modules).
- To build it, run `make` from the plugin root dir. This will create a new `go.plugin` binary that includes your newly
  developed collector. It will be placed into the `bin` directory (e.g `go.plugin/bin`)
- Run it in the debug mode `bin/goplugin -d -m <MODULE_NAME>`. This will output the `STDOUT` of the collector, the same
...
- If you want to test the collector with the actual Khulnasoft Agent, you need to replace the `go.plugin` binary that
  ...
  it, creating all the charts. It is advised not to remove the default `go.plugin` binary, but simply rename it
  to `go.plugin.old` so that the Agent doesn't run it, but you can easily rename it back once you are done.
...
Use [`agent/module`](https://github.com/khulnasoft/go.plugin/blob/master/agent/module/charts.go) package to create them,
...
> :exclamation: See the example [`example.go`](https://github.com/khulnasoft/go.plugin/blob/master/modules/example/example.go).
...
> :exclamation: See the example: [`charts.go`](https://github.com/khulnasoft/go.plugin/blob/master/modules/example/charts.go).

Put charts, charts templates and charts constructor functions in this file.

### File `init.go`

> :exclamation: See the example: [`init.go`](https://github.com/khulnasoft/go.plugin/blob/master/modules/example/init.go).
...
> :exclamation: See the example: [`collect.go`](https://github.com/khulnasoft/go.plugin/blob/master/modules/example/collect.go).
...
> :exclamation: See the example: [`example_test.go`](https://github.com/khulnasoft/go.plugin/blob/master/modules/example/example_test.go).
...
There are [some helper packages](https://github.com/khulnasoft/go.plugin/tree/master/pkg) for writing a module.
