<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/snmp/README.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/snmp/metadata.yaml"
...
`go.plugin` reports the time it took for the SNMP device to respond when executed in the debug mode.
...
The configuration file name for this integration is `go/snmp.conf`.
...
To troubleshoot issues with the `snmp` collector, run the `go.plugin` with the debug option enabled. The output
