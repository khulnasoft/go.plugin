<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/powerdns_recursor/README.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/powerdns_recursor/metadata.yaml"
...
The configuration file name for this integration is `go/powerdns_recursor.conf`.
...
sudo ./edit-config go/powerdns_recursor.conf
```
#### Options
...
To troubleshoot issues with the `powerdns_recursor` collector, run the `go.plugin` with the debug option enabled. The output
...
- Run the `go.plugin` to debug the collector:
  ...
  ```bash
  ./go.plugin -d -m powerdns_recursor
  ```
