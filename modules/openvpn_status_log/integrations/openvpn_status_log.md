<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/openvpn_status_log/README.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/openvpn_status_log/metadata.yaml"
...
The configuration file name for this integration is `go/openvpn_status_log.conf`.
...
sudo ./edit-config go/openvpn_status_log.conf
```
#### Options
...
To troubleshoot issues with the `openvpn_status_log` collector, run the `go.plugin` with the debug option enabled. The output
...
- Run the `go.plugin` to debug the collector:
  ...
  ```bash
  ./go.plugin -d -m openvpn_status_log
  ```
