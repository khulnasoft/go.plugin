<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/weblog/README.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/weblog/metadata.yaml"
...
The configuration file name for this integration is `go/web_log.conf`.
...
sudo ./edit-config go/web_log.conf
```
#### Options
...
| url_patterns.pattern | Used to match against full original request URI. Pattern syntax in [matcher](https://github.com/khulnasoft/go.plugin/tree/master/pkg/matcher#supported-format). |  | yes |
...
To troubleshoot issues with the `web_log` collector, run the `go.plugin` with the debug option enabled. The output
