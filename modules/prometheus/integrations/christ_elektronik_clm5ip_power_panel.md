<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/prometheus/integrations/christ_elektronik_clm5ip_power_panel.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/prometheus/metadata.yaml"
...
The full list of endpoints is available in the collector's [configuration file](https://github.com/khulnasoft/go.plugin/blob/master/config/go/prometheus.conf).
...
The configuration file name for this integration is `go/prometheus.conf`.
...
sudo ./edit-config go/prometheus.conf
```
#### Options
...
- Pattern syntax: [selector](https://github.com/khulnasoft/go.plugin/blob/master/pkg/prometheus/selector/README.md).
- Option syntax:
...
To troubleshoot issues with the `prometheus` collector, run the `go.plugin` with the debug option enabled. The output
