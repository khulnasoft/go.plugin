<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/postgres/README.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/postgres/metadata.yaml"
...
The configuration file name for this integration is `go/postgres.conf`.
...
sudo ./edit-config go/postgres.conf
```
#### Options
...
| collect_databases_matching | Databases selector. Determines which database metrics will be collected. Syntax is [simple patterns](https://github.com/khulnasoft/go.plugin/tree/master/pkg/matcher#simple-patterns-matcher). |  | no |
...
To troubleshoot issues with the `postgres` collector, run the `go.plugin` with the debug option enabled. The output
