<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/ping/README.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/ping/metadata.yaml"
sidebar_label: "Ping"
...
- privileged (send raw ICMP ping, default). Requires
  ...
  ```bash
  sudo setcap CAP_NET_RAW=eip <INSTALL_PREFIX>/usr/libexec/khulnasoft/plugins.d/go.plugin
  ```
...
The configuration file name for this integration is `go/ping.conf`.
...
To troubleshoot issues with the `ping` collector, run the `go.plugin` with the debug option enabled. The output
