<!--startmeta
custom_edit_url: "https://github.com/khulnasoft/go.plugin/edit/master/modules/nvidia_smi/README.md"
meta_yaml: "https://github.com/khulnasoft/go.plugin/edit/master/modules/nvidia_smi/metadata.yaml"
...
This collector is disabled by default. You need to explicitly enable it in the `go.conf` file.
...
The configuration file name for this integration is `go/nvidia_smi.conf`.
...
sudo ./edit-config go/nvidia_smi.conf
```
#### Options
...
To troubleshoot issues with the `nvidia_smi` collector, run the `go.plugin` with the debug option enabled. The output
