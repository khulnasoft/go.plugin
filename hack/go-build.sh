...
build() {
  ...
  CGO_ENABLED=0 GOOS="$1" GOARCH="$2" go build -ldflags "${GOLDFLAGS}" -o "$3" "github.com/khulnasoft/go.plugin/cmd/goplugin"
}

create_config_archives() {
  mkdir -p bin
  tar -zcvf "bin/config.tar.gz" -C config .
  tar -zcvf "bin/go.plugin-config-${VERSION}.tar.gz" -C config .
}

create_vendor_archives() {
  mkdir -p bin
  go mod vendor
  tar -zc --transform "s:^:go.plugin-${VERSION#v}/:" -f "bin/vendor.tar.gz" vendor
  tar -zc --transform "s:^:go.plugin-${VERSION#v}/:" -f "bin/go.plugin-vendor-${VERSION}.tar.gz" vendor
}
...
build_all_platforms() {
  for PLATFORM in "${PLATFORMS[@]}"; do
    ...
    FILE="bin/go.plugin-${VERSION}.${GOOS}-${GOARCH}"
...
build_specific_platform() {
  ...
  build "$GOOS" "$GOARCH" bin/goplugin
}
